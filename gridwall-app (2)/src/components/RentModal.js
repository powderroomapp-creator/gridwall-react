...
const HOT_ZONES = [
  { zone: 'A', label: 'Center Core', demand: 'Very High', available: 4200, marketRate: '$0.25/sq/wk', color: '#fff' },
  { zone: 'B', label: 'Mid Ring', demand: 'High', available: 48000, marketRate: '$0.20/sq/wk', color: '#ccc' },
  { zone: 'C', label: 'Outer Ring', demand: 'Moderate', available: 182000, marketRate: '$0.15/sq/wk', color: '#888' },
]
...
export default function RentModal({ onClose }) {
  const [tab, setTab] = useState('market') // market | past
  const [zone, setZone] = useState('a')
  const [squares, setSquares] = useState(300)
  const [weeks, setWeeks] = useState(1)

  const zoneData = HOT_ZONES.find(z => z.zone.toLowerCase() === zone)
  const ratePerSq = zone === 'a' ? 0.25 : zone === 'b' ? 0.20 : 0.15
  const total = (squares * ratePerSq * weeks).toFixed(2)
  const yourCut = (squares * ratePerSq * weeks * 0.80).toFixed(2)
  ...
  return (
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal" style={{ maxWidth: 560 }}>
        ...
        <div style={{ display:'flex', gap:'0', marginBottom:'1.5rem', border:'1px solid rgba(255,255,255,0.12)' }}>
          {[['market','Market'], ['past','Past Rentals']].map(([id, label]) => (
            <button key={id} onClick={() => setTab(id)} style={{
              flex:1, padding:'0.65rem', background: tab===id ? '#fff' : 'transparent',
              color: tab===id ? '#000' : '#9e9e9e', border:'none',
        ...
        {tab === 'market' && (
          <>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:'0.5rem', marginBottom:'1.5rem' }}>
              {HOT_ZONES.map(z => (
                <div key={z.zone} onClick={() => setZone(z.zone.toLowerCase())} style={{
                  border: `1px solid ${zone === z.zone.toLowerCase() ? '#fff' : 'rgba(255,255,255,0.12)'}`,
                  padding:'0.9rem 1.25rem', cursor:'pointer',
                  background: zone === z.zone.toLowerCase() ? 'rgba(255,255,255,0.04)' : 'transparent',
                    ...
                    <div>
                      ...
                      <div style={{ display:'flex', alignItems:'center', gap:'0.5rem' }}>
                        <span style={{
                          ...
                          padding:'1px 6px',
                          background: z.demand === 'Very High' ? 'rgba(255,255,255,0.15)' : z.demand === 'High' ? 'rgba(255,255,255,0.08)' : 'rgba(255,255,255,0.04)',
                          color: z.demand === 'Very High' ? '#fff' : '#9e9e9e'
                        }}>{z.demand} Demand</span>
            ...
            <div style={{ background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.1)', padding:'1rem 1.25rem', marginBottom:'1.25rem' }}>
              ...
              <div style={{ display:'flex', justifyContent:'space-between', fontSize:'0.75rem' }}>
                ...
                <span style={{fontFamily:'Space Mono,monospace', color:'#555'}}>${yourCut}</span>
              </div>
              <div style={{ marginTop:'0.75rem', paddingTop:'0.75rem', borderTop:'1px solid rgba(255,255,255,0.08)', fontSize:'0.75rem', color:'#9e9e9e' }}>
                Approx. {Math.round(Math.sqrt(squares))}×{Math.round(Math.sqrt(squares))} pixel area on the grid
              </div>
            </div>
          ...
          <>
                ...
                <div key={i} style={{ padding:'0.9rem 0', borderBottom:'1px solid rgba(255,255,255,0.06)', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                  <div>
                    ...
                    <div style={{ fontFamily:'Space Mono,monospace', fontSize:'0.65rem', color:'#555' }}>
                      Zone {r.zone} · {r.squares.toLocaleString()} sq · {r.weeks} wk{r.weeks>1?'s':''}
                    </div>
                  </div>
            ...
            <div style={{ marginTop:'1.25rem', padding:'1rem', background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.08)' }}>
              ...
              <div style={{ display:'flex', gap:'1.5rem', flexWrap:'wrap' }}>
                {[['$415','Avg campaign spend'],['487','Avg cluster size'],['2.4 wks','Avg duration']].map(([v,l]) => (
                  <div key={l}>
