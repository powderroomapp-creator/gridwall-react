    ...
    <div className="modal-overlay">
      <div className="modal" style={{ maxWidth: step === 2 ? '600px' : '480px' }}>
        <div style={{ display:'flex', gap:'0.5rem', marginBottom:'1.5rem' }}>
          {[1,2,3].map(s => (
            <div key={s} style={{ flex:1, height:'2px', background: s <= step ? '#fff' : 'rgba(255,255,255,0.15)', transition:'background 0.3s' }} />
          ))}
        </div>
        ...
        {step === 1 && (
          <>
            ...
            {avatarUrl && <div style={{ textAlign:'center', marginBottom:'1.5rem' }}><img src={avatarUrl} alt="avatar" style={{ width:72, height:72, borderRadius:'50%', objectFit:'cover', border:'2px solid rgba(255,255,255,0.2)' }} /></div>}
            <div className="input-group"><label>Username</label><input placeholder="yourhandle" value={profile.username} onChange={e => setProfile({...profile, username:e.target.value})} /></div>
            <div className="input-group"><label>Bio</label><textarea placeholder="What are you about?" rows={3} value={profile.bio} onChange={e => setProfile({...profile, bio:e.target.value})} style={{ width:'100%', background:'rgba(255,255,255,0.05)', border:'1px solid rgba(255,255,255,0.12)', padding:'0.75rem 1rem', color:'#fff', resize:'none', outline:'none' }} /></div>
            ...
            <div className="input-group" style={{marginBottom:'0.5rem'}}><label>Instagram</label><input placeholder="@yourhandle" value={profile.instagram} onChange={e => setProfile({...profile, instagram:e.target.value})} /></div>
            <div className="input-group" style={{marginBottom:'0.5rem'}}><label>TikTok</label><input placeholder="@yourhandle" value={profile.tiktok} onChange={e => setProfile({...profile, tiktok:e.target.value})} /></div>
            <div className="input-group" style={{marginBottom:'1.5rem'}}><label>Facebook</label><input placeholder="facebook.com/yourname" value={profile.facebook} onChange={e => setProfile({...profile, facebook:e.target.value})} /></div>
        ...
        {step === 2 && (
          <>
            ...
            <p style={{ color:'#9e9e9e', fontSize:'0.85rem', marginBottom:'0.5rem' }}>Zone A is <span style={{color:'#fff', fontWeight:600}}>79% sold.</span> Center squares won't last.</p>
            <p style={{ fontFamily:'Space Mono,monospace', fontSize:'0.65rem', color:'#555', marginBottom:'1.5rem' }}>247,483 SQUARES CLAIMED — 752,517 REMAINING</p>
            <div style={{ display:'flex', flexDirection:'column', gap:'0.75rem', marginBottom:'1.5rem' }}>
              {PACKAGES.map(pkg => (
                <div key={pkg.id} onClick={() => setSelectedPkg(pkg.id)} style={{ border:`1px solid ${selectedPkg===pkg.id?'#fff':'rgba(255,255,255,0.12)'}`, padding:'1.25rem', cursor:'pointer', background: pkg.featured&&selectedPkg===pkg.id?'rgba(255,255,255,0.06)':'transparent', position:'relative', transition:'border-color 0.2s' }}>
            ...
            <div style={{ display:'flex', gap:'0.75rem' }}>
              ...
              <button className="btn-primary" onClick={() => setStep(3)}>Continue — ${PACKAGES.find(p=>p.id===selectedPkg)?.price}</button>
            </div>
            <button onClick={() => setStep(3)} style={{ background:'none', border:'none', color:'#555', fontSize:'0.75rem', width:'100%', marginTop:'0.75rem', textDecoration:'underline', cursor:'pointer' }}>Skip for now</button>
          </>
        )}
        ...
        {step === 3 && (
          <>
            ...
            <p style={{ color:'#9e9e9e', fontSize:'0.85rem', marginBottom:'1.5rem' }}>Review your setup before we drop you on the grid.</p>
            ...
            <div style={{ display:'flex', gap:'0.75rem' }}>
              ...
              <button className="btn-primary" onClick={saveProfile} disabled={saving}>{saving ? 'Saving...' : 'Enter GridWall →'}</button>
            </div>
          </>
        )}
      </div>
