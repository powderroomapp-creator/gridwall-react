    ...
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal" style={{ maxWidth:520 }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:'0.5rem', marginBottom:'1.5rem' }}>
          {MOCK_OWNED.map(sq => (
            <div key={sq.id} onClick={() => { setSelected(sq.id); setAskPrice(''); setQty(1) }} style={{ border:`1px solid ${selected===sq.id?'#fff':'rgba(255,255,255,0.12)'}`, padding:'1rem 1.25rem', cursor:'pointer', background:selected===sq.id?'rgba(255,255,255,0.04)':'transparent', transition:'border-color 0.2s' }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                ...
                <div style={{ textAlign:'right' }}>
                  <div style={{ fontFamily:'Space Mono,monospace', fontSize:'0.9rem', fontWeight:700 }}>${sq.currentValue}</div>
          ...
          <>
            <div style={{ display:'flex', gap:'0.75rem', marginBottom:'1rem' }}>
              <div className="input-group" style={{flex:1, marginBottom:0}}>
                ...
                <input type="number" min={1} max={MOCK_OWNED.find(s=>s.id===selected)?.count||1} value={qty} onChange={e => setQty(+e.target.value)} />
              </div>
              <div className="input-group" style={{flex:1, marginBottom:0}}>
                <label>Ask price ($)</label>
                <input type="number" min={0.01} step={0.01} placeholder="e.g. 5.00" value={askPrice} onChange={e => setAskPrice(e.target.value)} />
              </div>
            </div>
            {askPrice && (
            ...
            <button className="btn-primary" onClick={() => alert('Listing going live!')}>
        ...
        <div style={{ marginTop:'1.5rem', padding:'1rem', border:'1px solid rgba(255,255,255,0.08)', background:'rgba(255,255,255,0.02)' }}>
          ...
          <div style={{ display:'flex', gap:'1.5rem', flexWrap:'wrap' }}>
            {[['$2.40','Avg Zone C'],['$8.50','Avg Zone B'],['$38','Avg Zone A']].map(([v,l]) => (
              <div key={l}>
