...
const PACKAGES = [
  { id: 'single', label: '1 Square', count: 1, price: 1, desc: 'Own a single square. Your profile, your link.' },
  { id: 'block', label: '10×10 Block', count: 100, price: 100, desc: 'Spell words. Build logos. Earn rental income.', featured: true },
  { id: 'district', label: '100×100 District', count: 10000, price: 1000, desc: 'Own a neighborhood. Maximum visibility and income.' },
]
  ...
  return (
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal" style={{ maxWidth:560 }}>
        ...
        <p style={{ fontFamily:'Space Mono,monospace', fontSize:'0.65rem', color:'#f55', letterSpacing:'0.1em', marginBottom:'1.25rem' }}>⚡ ZONE A IS 79% SOLD — CENTER SQUARES RUNNING OUT</p>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:'0.5rem', marginBottom:'1.5rem' }}>
          {PACKAGES.map(p => (
            <div key={p.id} onClick={() => setSelected(p.id)} style={{ border:`1px solid ${selected===p.id?'#fff':'rgba(255,255,255,0.12)'}`, padding:'1rem 1.25rem', cursor:'pointer', position:'relative', background:selected===p.id?'rgba(255,255,255,0.04)':'transparent', transition:'border-color 0.2s' }}>
              {p.featured && <div style={{ position:'absolute', top:'-1px', right:'1rem', background:'#fff', color:'#000', fontFamily:'Space Mono,monospace', fontSize:'0.55rem', fontWeight:700, padding:'2px 8px' }}>MOST POPULAR</div>}
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                <div>
                  <div style={{ fontWeight:700, marginBottom:'0.2rem' }}>{p.label}</div>
                  <div style={{ fontSize:'0.8rem', color:'#9e9e9e' }}>{p.desc}</div>
                </div>
                <div style={{ fontFamily:'Space Mono,monospace', fontSize:'1.2rem', fontWeight:700, marginLeft:'1rem', flexShrink:0 }}>${p.price}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:'0.5rem', marginBottom:'1.5rem' }}>
          {ZONES.map(z => (
            <div key={z.id} onClick={() => setZone(z.id)} style={{ border:`1px solid ${zone===z.id?'#fff':'rgba(255,255,255,0.12)'}`, padding:'0.9rem 1.25rem', cursor:'pointer', background:zone===z.id?'rgba(255,255,255,0.04)':'transparent', transition:'border-color 0.2s' }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                <div>
        ...
        <button className="btn-primary" onClick={() => alert('Stripe coming next!')}>
          Buy {pkg.count.toLocaleString()} Square{pkg.count>1?'s':''} — ${pkg.price}
        </button>
        <p style={{ fontFamily:'Space Mono,monospace', fontSize:'0.6rem', color:'#555', textAlign:'center', marginTop:'0.75rem' }}>Secured by Stripe · Instant ownership · Resellable anytime</p>
      </div>
    </div>
  )
}
